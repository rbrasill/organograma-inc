...
// "219 - Emotion III" (e variações sem espaço, "127- Unique Benfica") — o
// número é o mesmo da série oficial LOCTRA do banco (219 ↔ LOCTRA219).
...
export function localComCodigo(texto) {
  ...
  if (!m) return null;
  return { codigo: `LOCTRA${m[1]}`, nome: m[2].trim() };
}
...
export function extrairLinhas(matriz) {
  ...
  for (let r = 1; r < matriz.length; r++) {
    ...
    // local no formato novo ("219 - Emotion III"): deriva o código LOCTRA do
    // prefixo quando o arquivo não traz a coluna de código separada
    const localTxt = cel(row, "local");
    const locParse = cel(row, "codigoLocal") ? null : localComCodigo(localTxt);
    linhas.push({
      ...
      local: localTxt,
      codigoLocal: cel(row, "codigoLocal") || (locParse ? locParse.codigo : ""), // v2: LOCTRA… · v3: derivado do prefixo
